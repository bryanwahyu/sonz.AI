...
## Sand-ai Documentation
- Use the architecture plan below when you need guidance on domain boundaries, scaling patterns, or operational runbooks for Sand-ai.
- [Sand-ai Architecture & Scale Plan](docs/sand-ai-architecture.md)

## Sand-ai Quickstart
- Populate the secret placeholders referenced in `deploy/sandai/nakama-config.yml` (`STEAM_PUBLISHER_KEY`, `FACEBOOK_APP_ID`, etc.) via your shell exports or a compose-compatible `.env` file before booting the stack.
- Build and launch the local Sand-ai stack (CockroachDB, Nakama, façade API, Prometheus) with:

  ```shell
  docker compose -f deploy/sandai/docker-compose.yml up --build
  ```
